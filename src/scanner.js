...
export class StructuralArbitrageScanner {
  constructor(config) { this.config = config; this.gamma = new GammaClient(config.gammaBaseUrl, config.requestTimeoutMs); this.clob = new ClobClient(config.clobBaseUrl, config.requestTimeoutMs); }
  async scan() {
    const nowMs = Date.now();
    const markets = await this.gamma.listActiveBinaryMarkets(this.config.maxMarkets, this.config.marketPageSize, this.config.marketStartOffset ?? 0);

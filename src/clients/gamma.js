...
function normalizeMarket(raw) {
  ...
  const market = {
    id: String(raw.id ?? raw.conditionId), conditionId: raw.conditionId, question: raw.question, slug: raw.slug,
    yesTokenId, noTokenId, active: raw.active ?? false, closed: raw.closed ?? false,
    acceptingOrders: raw.acceptingOrders ?? false, negRisk: raw.negRisk ?? false, feesEnabled: raw.feesEnabled ?? false
  };
...
export class GammaClient {
  ...
  async listActiveBinaryMarkets(maxMarkets, pageSize, startOffset = 0) {
    const markets = [];

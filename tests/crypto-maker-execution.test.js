...
function run(records) {
  ...
  fs.writeFileSync(input, zlib.gzipSync(raw));
...
test('maker simulator requires displayed queue consumption before fill', () => {
  const records = [
    { kind: 'recorder_start', received_at: iso() },
    { kind: 'connection_open', received_at: iso(), connection_id: 1 },
    ...
    {
      kind: 'coinbase_message', received_at: iso(15, 30), connection_id: 1,
      payload: {
    ...
    { kind: 'recorder_stop', received_at: iso(76, 1) }
  ];
...
test('forward level2 sequence gap invalidates open hypothetical orders', () => {
  const records = [
    ...
    { kind: 'recorder_stop', received_at: iso(17) }
  ];

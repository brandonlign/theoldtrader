...
spec=importlib.util.spec_from_file_location('t30src32',P)
...
def prepare(manifest,mode,coin,out,meta):
    ...
    if m.get('experimentId')!='proactive-alt-basket-crossvenue-v1' or m.get('trialNumber')!=32 or m.get('status')!='FROZEN_PRE_DEVELOPMENT': raise RuntimeError('wrong frozen Trial 32 manifest')
    if coin not in m['assets']: raise RuntimeError('asset not frozen')
    if mode=='final':
        g=Path('research/crypto/results/proactive-alt-basket-crossvenue-v1-development/gate.json')
...
def evaluate(manifest,mode,inputs,out):
    ...
    obj={'experimentId':m['experimentId'],'trialNumber':32,'mode':mode,'generatedAt':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'basket':x,'sleeves':per,'implementation':'proactive-alt-basket-90pct-v1','realMoneyAllowed':False}; Path(out).parent.mkdir(parents=True,exist_ok=True); Path(out).write_text(json.dumps(obj,indent=2)+'\n'); print(json.dumps(obj,indent=2))

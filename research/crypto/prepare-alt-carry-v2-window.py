#!/usr/bin/env python3
"""Acquire one frozen Trial 18 BNB/XRP carry window without economics."""
...
FINAL_GATE=Path('research/crypto/results/alt-carry-basket-v2-development/gate.json')
def load_module(name,path):
 spec=importlib.util.spec_from_file_location(name,path)
 if spec is None or spec.loader is None: raise RuntimeError(f'Unable to import {path}')
 mod=importlib.util.module_from_spec(spec); sys.modules[name]=mod; spec.loader.exec_module(mod); return mod
def main():
 if len(sys.argv)<6: raise SystemExit('usage: prepare-alt-carry-v2-window.py <manifest> <development|final> <symbol> <out.csv> <sources.json> [--confirm-final YES]')
 ...
 if out.exists() or source.exists(): raise RuntimeError('Trial 18 output already exists')
 m=json.loads(manifest_path.read_text())
 if m.get('experimentId')!='alt-carry-basket-v2' or m.get('trialNumber')!=18 or m.get('status')!='FROZEN_PRE_DEVELOPMENT': raise RuntimeError('Expected frozen Trial 18')
 if symbol not in m['assetSelection']['symbols']: raise RuntimeError('symbol outside frozen Trial 18 set')
 if mode=='development':
  ...
  if not FINAL_GATE.exists(): raise RuntimeError('Trial 18 final gate absent')
  gate=json.loads(FINAL_GATE.read_text())
  if gate.get('experimentId')!=m['experimentId'] or gate.get('developmentGatePass') is not True: raise RuntimeError('Trial 18 development did not authorize final')
 ...
 original=load_module(f'trial18_base_{symbol.lower()}',BASE_BUILDER); union=load_module(f'trial18_union_{symbol.lower()}',UNION_BUILDER)
 original.SYMBOL=symbol; union.SYMBOL=symbol
 ...
 if len(times)!=expected or not times or times[0]!=start_ms or times[-1]!=end_ms-EIGHT_HOURS_MS: raise RuntimeError(f'{symbol} funding grid coverage failure')

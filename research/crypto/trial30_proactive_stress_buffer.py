...
spec=importlib.util.spec_from_file_location('t27src30',P)
...
def prepare(manifest,mode,coin,out,meta):
    ...
    if m.get('experimentId')!='proactive-stress-buffer-crossvenue-v1' or m.get('trialNumber')!=30 or m.get('status')!='FROZEN_PRE_DEVELOPMENT': raise RuntimeError('wrong frozen Trial 30 manifest')
    if coin not in m['assets']: raise RuntimeError('asset not frozen')
    if mode=='final':
        g=Path('research/crypto/results/proactive-stress-buffer-crossvenue-v1-development/gate.json')
...
def equalize(bb,hb,cash,eq,c,tf):
    tb=eq*c['targetBinanceCollateralPct']; th=eq*c['targetHyperliquidCollateralPct']; tc=eq*c['targetCashReservePct']
    moved=.5*(abs(tb-bb)+abs(th-hb)+abs(tc-cash)); fee=moved*tf; eq2=eq-fee
    return eq2*c['targetBinanceCollateralPct'],eq2*c['targetHyperliquidCollateralPct'],eq2*c['targetCashReservePct'],moved,fee
...
def simulate(rows,start,m):
    ...
    trading=transfer=transferred=0.0; proactive=0; reanchors=0; breach=None; stress=True; min_pre_buffer=float('inf'); min_post_buffer=float('inf')
    maint=mm['maintenanceMarginPctOfLegNotional']; gap=mm['unilateralGapStressPct']; extra=s['proactiveStressBufferPctOfLegNotional']
    for r in rows:
        if prev is not None:
        ...
        pre=min(bb-(gap+maint)*nb,hb-(gap+maint)*nh); denom=max(nb,nh,1e-12); min_pre_buffer=min(min_pre_buffer,pre/denom)
        trigger=(bb-(gap+maint)*nb<extra*nb) or (hb-(gap+maint)*nh<extra*nh)
        if trigger:
            eq=cash+bb+hb; bb,hb,cash,moved,fee=equalize(bb,hb,cash,eq,c,tf); transferred+=moved; transfer+=fee; proactive+=1
            nb=abs(units*r['pb']); nh=abs(units*r['ph'])
        post=min(bb-(gap+maint)*nb,hb-(gap+maint)*nh); min_post_buffer=min(min_post_buffer,post/max(nb,nh,1e-12))
    ...
    z=b.stats(series,start); z.update({'marginBreach':breach,'allGapStressPass':stress,'active':bool(rows),'proactiveCollateralRebalances':proactive,'riskTriggeredReanchors':reanchors,'grossCollateralTransferred':transferred,'transferCosts':transfer,'tradingCosts':trading,'totalModeledCosts':transfer+trading,'minimumPreTransferStressExcessPctOfMaxLegNotional':None if min_pre_buffer==float('inf') else min_pre_buffer,'minimumPostTransferStressExcessPctOfMaxLegNotional':None if min_post_buffer==float('inf') else min_post_buffer,'settlementCountDistribution':dict(sorted(Counter(r['hc'] for r in rows).items()))}); return z,series
...
def evaluate(manifest,mode,inputs,out):
    ...
    obj={'experimentId':m['experimentId'],'trialNumber':30,'mode':mode,'generatedAt':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'basket':x,'sleeves':per,'implementation':'proactive-stress-buffer-collateral-v1','realMoneyAllowed':False}; Path(out).parent.mkdir(parents=True,exist_ok=True); Path(out).write_text(json.dumps(obj,indent=2)+'\n'); print(json.dumps(obj,indent=2))

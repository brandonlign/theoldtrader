...
def main():
    ...
    if out.exists() or source.exists():
        raise RuntimeError('Trial 19 output already exists')
    m = json.loads(manifest_path.read_text())
    if m.get('experimentId') != 'funding-regime-carry-v1' or m.get('trialNumber') != 19 or m.get('status') != 'FROZEN_PRE_DEVELOPMENT':
        raise RuntimeError('Expected frozen Trial 19')
    if symbol not in m['assetSelection']['symbols']:
        raise RuntimeError('symbol outside frozen Trial 19 set')
    if mode == 'development':
    ...
    original = load_module(f'trial19_base_{symbol.lower()}', BASE_BUILDER)
    union = load_module(f'trial19_union_{symbol.lower()}', UNION_BUILDER)
    ...
    expected = (end_ms - start_ms) // EIGHT_HOURS_MS
    ...
    original.write_csv(out, synchronized)

#!/usr/bin/env python3
"""Acquire one frozen Trial 19 ADA/DOGE funding-regime carry window without economics."""
...
FINAL_GATE = Path('research/crypto/results/funding-regime-carry-v1-development/gate.json')

def load_source_lib():
    spec = importlib.util.spec_from_file_location('trial19_carry_source_union', SOURCE_LIB)
...
def main():
    if len(sys.argv) < 6:
        raise SystemExit('usage: prepare-funding-regime-carry-window.py <manifest> <development|final> <symbol> <out.csv> <sources.json> [--confirm-final YES]')
    ...
    if out.exists() or source.exists(): raise RuntimeError('Trial 19 output already exists')
    m=json.loads(manifest_path.read_text())
    if m.get('experimentId')!='funding-regime-carry-v1' or m.get('trialNumber')!=19 or m.get('status')!='FROZEN_PRE_DEVELOPMENT': raise RuntimeError('Expected frozen Trial 19')
    if symbol not in m['assetSelection']['symbols']: raise RuntimeError('symbol outside frozen Trial 19 set')
    if mode=='development':
        ...
        if not FINAL_GATE.exists(): raise RuntimeError('Trial 19 final gate absent')
        gate=json.loads(FINAL_GATE.read_text())
        if gate.get('experimentId')!=m['experimentId'] or gate.get('developmentGatePass') is not True: raise RuntimeError('Trial 19 development did not authorize final')
        window=m['finalHoldout']
    start,end=window['startInclusive'],window['endExclusive']
    start_ms=int(datetime.fromisoformat(start.replace('Z','+00:00')).timestamp()*1000); end_ms=int(datetime.fromisoformat(end.replace('Z','+00:00')).timestamp()*1000)
    ...
    lib=load_source_lib(); days=list(lib.day_range(start_ms,end_ms)); months=list(lib.month_range(start_ms,end_ms))

...
function validate(result) {
  if (result?.experimentId !== "cross-venue-funding-v1" || result?.trialNumber !== 7) throw new Error("Unexpected Trial 7 report input identity");
  if (result.paperOnly !== true || result.livePromotionAllowed !== false) throw new Error("Trial 7 report input must remain paper-only and non-live");
  if (!result.classification || !result.dataGate) throw new Error("Trial 7 report input is incomplete");
}

function decompositionResidual(result) {
  const funding = number(result.primary?.fundingPnl?.net);
  const basis = number(result.primary?.pricePnl?.combinedBasisBeforeFriction);
  const friction = number(result.primary?.executionFriction?.totalUsd);
  const net = number(result.primary?.netPnl);
  if ([funding, basis, friction, net].some((value) => value === null)) return null;
  return funding + basis - friction - net;
}
...
function reportMarkdown(result) {
  ...
  lines.push(
    ...
    `- Sortino (zero target): ${number(result.primary?.stats?.sortino)?.toFixed(3) ?? "n/a"}`,
    `- Max drawdown from pre-entry equity: ${pct(result.primary?.stats?.maxDrawdown)}`,
    ...
    `- Raw cross-venue basis P&L before friction: ${usd(result.primary?.pricePnl?.combinedBasisBeforeFriction)}`,
    `- Total modeled execution friction: ${usd(result.primary?.executionFriction?.totalUsd)}`,
    `- Diagnostic basis/leg P&L after friction: ${usd(result.primary?.pricePnl?.combinedBasisAfterFriction)}`,
    `- Funding + raw basis − friction − reported net residual: ${usd(decompositionResidual(result))}`,
  ...
  for (const window of result.windows60d ?? []) {
    lines.push(`- ${window.start} → ${window.end}: ${usd(window.pnl)} (${window.complete === false ? "incomplete" : window.positive ? "positive" : "non-positive"})`);
  }
  if (result.consistencyTelescopeErrorUsd !== undefined) {
    lines.push(`- Window telescope residual vs full P&L: ${usd(result.consistencyTelescopeErrorUsd)}`);
  }
  lines.push(
    "", "## Comparator", "",
...
export function buildCrossVenueFundingReport(result) {
  ...
  files["economics.csv"] = csv([
    ...
    ["gross_pnl_before_friction_usd", result.primary?.grossPnlBeforeFriction, result.costStress?.grossPnlBeforeFriction],
    ...
    ["sortino_zero_target", result.primary?.stats?.sortino, result.costStress?.stats?.sortino],
    ...
    ["raw_basis_pnl_before_friction_usd", result.primary?.pricePnl?.combinedBasisBeforeFriction, result.costStress?.pricePnl?.combinedBasisBeforeFriction],
    ["diagnostic_basis_pnl_after_friction_usd", result.primary?.pricePnl?.combinedBasisAfterFriction, result.costStress?.pricePnl?.combinedBasisAfterFriction],
    ["execution_friction_usd", result.primary?.executionFriction?.totalUsd, result.costStress?.executionFriction?.totalUsd],
    ["decomposition_residual_usd", decompositionResidual(result), ""],
    ["break_even_friction_bps_per_order", result.breakEvenAllInFrictionBpsPerOrder, ""]
  ]);

  files["windows-60d.csv"] = csv([
    ["start", "end", "pnl_usd", "positive", "complete"],
    ...(result.windows60d ?? []).map((window) => [window.start, window.end, window.pnl, window.positive, window.complete])
  ]);

  files["margin-stress.csv"] = csv([
    ["shock_fraction", "breached", "minimum_binance_excess_usd", "minimum_hyperliquid_excess_usd"],
    ...Object.entries(result.primary?.margin?.stress ?? {}).map(([shock, state]) => [shock, state.breached, state.minBinanceExcess, state.minHyperliquidExcess])
  ]);

  const equitySvg = svgSeries(result.primary?.equitySeries ?? [], { title: "Trial 7 total-equity path", yKeys: ["equity"] });
  if (equitySvg) files["equity-curve.svg"] = equitySvg;
  const marginSvg = svgSeries(result.primary?.marginSeries ?? [], { title: "Trial 7 venue margin excess", yKeys: ["binanceExcess", "hyperliquidExcess"] });

...
// Post-evaluation reporting correction: metrics always anchor to the fixed $10,000 starting capital, so entry costs on day one are included. Trading decisions and fills are unchanged.
...
function metrics(st){ const d=dailyEquity(st.equity), rets=[]; let prev=STARTING_CASH; for(const point of d){ rets.push(prev>0?point.value/prev-1:0); prev=point.value; } const start=STARTING_CASH,end=d.at(-1)?.value??start,days=d.length>1?Math.max(1,(d.at(-1).time-d[0].time)/86400):1,ann=(end/start)**(365/days)-1,sd=stdev(rets),down=rets.filter(x=>x<0),ds=stdev(down); let peak=STARTING_CASH,mdd=0; for(const p of d){peak=Math.max(peak,p.value);mdd=Math.min(mdd,p.value/peak-1);} const wins=st.trades.filter(t=>t.pnl>0),loss=st.trades.filter(t=>t.pnl<0),gp=wins.reduce((s,t)=>s+t.pnl,0),gl=Math.abs(loss.reduce((s,t)=>s+t.pnl,0)); const exp=mean([STARTING_CASH,...d.map(p=>p.value)]); const exposure=mean(dailyEquity(st.exposure).map(p=>p.value)); return {netReturn:end/start-1,annualizedReturn:ann,sharpe:sd?Math.sqrt(365)*mean(rets)/sd:0,sortino:ds?Math.sqrt(365)*mean(rets)/ds:0,maxDrawdown:mdd,calmar:mdd<0?ann/Math.abs(mdd):null,winRate:st.trades.length?wins.length/st.trades.length:null,profitFactor:gl?gp/gl:(gp>0?null:0),expectancyPerTrade:st.trades.length?mean(st.trades.map(t=>t.pnl)):0,turnover:st.turnover,turnoverToAverageEquity:exp?st.turnover/exp:0,totalFees:st.fees,feeDrag:st.fees/start,averageExposure:exposure,closedTrades:st.trades.length,orderCount:st.orders,startValue:start,endValue:end,elapsedDays:days}; }
